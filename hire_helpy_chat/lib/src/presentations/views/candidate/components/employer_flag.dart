...
class EmployerFlag extends StatefulWidget {
  const EmployerFlag(
      {super.key, required this.country, this.marginValue, this.configKey, this.size});
...
class _EmployerFlagState extends State<EmployerFlag> {
  ...
  _getCountryData() async {
    print('data ${widget.configKey} ${widget.country}');
    List<Country> countryDataList = List<Country>.from(
        (await DBUtils.getKeyDataList(widget.configKey))
            .map((e) => Country.fromJson(e)));
    setState(() {
      flagUri = countryDataList.indexWhere((f) =>
                  f.name!.toLowerCase() == (widget.country).toLowerCase()) !=
              -1
          ? countryDataList[countryDataList.indexWhere((f) =>
                  f.name!.toLowerCase() == (widget.country).toLowerCase())]
              .flagUri
          : '';
    });
