...
class _ReferFriendSectionState extends State<ReferFriendSection> {
  ...
  Map? candidateData;
  List<kFlutterContacts.Contact> selectedContacts = [];
  ...
  Future _fetchContacts() async {
    if (!await kFlutterContacts.FlutterContacts.requestPermission(
        readonly: true)) {
      setState(() => _permissionDenied = true);
    } else {
      final contacts = await kFlutterContacts.FlutterContacts.getContacts(
          withProperties: true, withPhoto: true);
      setState(() => contactList = contacts);
    }
  }

  requestContactData() async {
    List<kFlutterContacts.Contact> data =
        await kFlutterContacts.FlutterContacts.getContacts(
            withProperties: true, withPhoto: true);
    setState(() {
      contactList = data;
    });
  ...
  void onContactSelected(kFlutterContacts.Contact contact) {
      ...
      child: ElevatedButton(
        ...
        child: Row(
          ...
          children: const [
            ...
            SizedBox(width: 8.0),
            Text(
              StringConst.sendInviteLinkText,
              style: TextStyle(
                fontSize: 14.0,
                ...
                child: Scrollbar(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      kFlutterContacts.Contact contact = contactList[index];
  ...
  bool isSelectedContact(kFlutterContacts.Contact contact) {
  ...
  Future<void> sendBulkSMSInvite() async {
    try {
      ...
      print('sendBulkSMSInvite error: $e');
    }
  }
