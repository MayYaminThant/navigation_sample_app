...
class _DropdownItemState extends State<DropdownItem> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            value == '' && required ? Border.all(color: AppColors.red) : null,
      ),
      child: Theme(
  ...
  Widget get _customDropDownButton => Center(
        child: Padding(
  ...
  //Gender Type
  Future<void> _getGenderTypesData() async {
    List<dynamic> value =
        await DBUtils.getKeyDataList(widget.prefix) as List<dynamic>;
    List<String> dataList = List<String>.from(value);
    setState(() {
      data.addAll(dataList);
    });
  }

  Text getItemText({String? item}) => Text(
        item != null ? item.tr : '',
