...
import 'package:dh_mobile/src/presentations/views/saved_search/components/rename_search_modal.dart';
...
class _SavedSearchCardState extends State<SavedSearchCard> {
      ...
      Row(
        ...
        children: [
          EmployerFlag(
            ...
            marginValue: 0.0,
            configKey: ConfigsKeyHelper.adSearchWorkCountryKey,
          ),
  ...
  Widget _buildSearchTime() {
    String searchOnTime = "-";
    try {
      final String createdAt = "${widget.savedSearch.createdAt}";
      final DateTime localTime =
      DateFormat('yyyy-MM-dd HH:mm:ss').parse(createdAt, true).toLocal();
      searchOnTime = DateFormat('yyyy-MM-dd').format(localTime);
    } catch (e) {
      searchOnTime = "-";
    }
  ...
  Widget _buildSalary() {
    return Column(children: [
      Row(children: [
        Text(
          '${widget.savedSearch.currency} ${widget.savedSearch.expectedSalary.toString()}  ',
          style: const TextStyle(
  ...
  Widget _buildInfoText() {
    return Column(
      ...
      children: [
        ...
        Text(
          '${StringConst.startDateText}: ${widget.savedSearch.startDate}',
          style: const TextStyle(
            color: AppColors.articleBackgroundColor,
            fontWeight: FontWeight.normal,
            fontSize: 12,
        ...
        const SizedBox(height: 4),
        Text(
          '${StringConst.restDaysText.tr}: ${widget.savedSearch.restDayChoice} ${widget.savedSearch.restDayChoice != null && widget.savedSearch.restDayChoice! > 1 ? 'Days' : 'Day'} (/Month)',
          style: const TextStyle(
            color: AppColors.articleBackgroundColor,
            fontWeight: FontWeight.normal,
            fontSize: 12,
    ...
    return PopupMenuButton<String>(
      ...
      onSelected: (String value) {
        ...
        if (value == 'expandedSearchValue') {
          ...
          //     context,
          //     MaterialPageRoute(
          //         builder: (context) => const AdvancedSearchSection()));
  ...
  _removeSaveSearch() {
    ...
    CandidateDeleteSaveSearchRequestParams params =
        CandidateDeleteSaveSearchRequestParams(
      saveSearchId: widget.savedSearch.id,
      token: candidateData!['token'],
    );
    candidateBloc.add(CandidateDeleteSaveSearchRequested(params: params));
  }
  ...
  _renameSaveSearch(String name) {
    ...
    CandidateUpdateSaveSearchRequestParams params =
        CandidateUpdateSaveSearchRequestParams(
            ...
            token: candidateData!['token'],
            name: name);
    candidateBloc.add(CandidateUpdateSaveSearchRequested(params: params));
  }

  _addSkills() {
    if (widget.savedSearch.skillGeneralHousework != null &&
        widget.savedSearch.skillGeneralHousework!) {
      _checkAddSkill('General Housework');
    }
    if (widget.savedSearch.skillInfantCare != null &&
        widget.savedSearch.skillInfantCare!) {
      _checkAddSkill('Infant Care');
    }
    if (widget.savedSearch.skillElderlyCare != null &&
        widget.savedSearch.skillElderlyCare!) {
      _checkAddSkill('Elderly Care');
    }
    if (widget.savedSearch.skillPetCare != null &&
        widget.savedSearch.skillPetCare!) {
      _checkAddSkill('Pet Care');
    }
    if (widget.savedSearch.skillCooking != null &&
        widget.savedSearch.skillCooking!) {
      _checkAddSkill('Cooking');
    }
    if (widget.savedSearch.skillSpecialNeedsCare != null &&
        widget.savedSearch.skillSpecialNeedsCare!) {
      _checkAddSkill('Special Needs Care');
    }
    if (widget.savedSearch.skillbedriddenCare != null &&
        widget.savedSearch.skillbedriddenCare!) {
      _checkAddSkill('Bedidden Care');
    }
    if (widget.savedSearch.skillHandicapCare != null &&
        widget.savedSearch.skillHandicapCare!) {
      _checkAddSkill('Handicap Care');
    }
    if (widget.savedSearch.skillOthersSpecify != null) {
      _checkAddSkill(widget.savedSearch.skillOthersSpecify!);
    }
  }
