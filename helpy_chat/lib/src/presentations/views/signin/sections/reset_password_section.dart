...
class _ResetPasswordSectionState extends State<ResetPasswordSection> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CandidateBloc, CandidateState>(builder: (_, state) {
      ...
      if (state is CandidateResetPasswordSuccess) {
        if (state.data.data != null) {
          showSuccessSnackBar(state.data.data!['message']);
          Get.offAllNamed(recoverSuccessPageRoute);
        }
      }

      if (state is CandidateResetPasswordFail) {
        if (state.message != '') {
  ...
  Widget get _getForgotPasswordScaffold {
    return Scaffold(
      body: SingleChildScrollView(
          ...
          child: _getForgotPasswordContainer),
    );
  }
    ...
    return Column(
      children: [
        ...
        const Padding(
            ...
            child: CustomImageView(image: 'assets/images/reset_password.png')),
        const Padding(
  ...
  _checkValidation() {
    if (newPasswordController.text.isEmpty) {
      ...
      showErrorSnackBar('The password and confirm password do not match.');
    } else {
  ...
  void _requestResetPassword() {
    ...
    superPrint('data $data');
    candidateBloc.add(CandidateResetPasswordRequested(params: params));
  }
}
